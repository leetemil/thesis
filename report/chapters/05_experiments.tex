...
\todo{discuss the probabilistic graph model of the problem. include that z removes any dependency between absolute positions in the protein sequence, i.e. we can calculate the probability of a protein conditioned on latent variable z by taking the product of the position probabilities. make figure with a single z with arrows to all positions x1, x2, ..., xn}
